...
├── logo-consensus.png         incluido       150 KB
...
El emblema va en la raíz con el nombre exacto **`logo-consensus.png`**. Se
incluye ya convertido a PNG de 512 × 512, listo para subir: no hace falta que
hagas nada con él salvo colocarlo junto a `index.html`.

La cabecera lo muestra a 48 px en escritorio y 44 px en móvil, con recorte
circular (`border-radius:50%`). El archivo original es cuadrado y con fondo
marino opaco; el recorte deja el disco limpio y, en tema oscuro, el corte ni se
aprecia porque el fondo del archivo y el de la superficie son exactamente el
mismo marino.

`tablero.js` prueba tres extensiones antes de rendirse —`.png`, `.jpg`,
`.svg`—, de modo que la cabecera no depende de con cuál hayas guardado el
archivo. Si ninguna responde, conmuta al logotipo tipográfico de reserva y la
cabecera sigue entera.

**Una limitación del emblema a este tamaño.** A 48 px, la tipografía perimetral
—CONSENSUS ESTRATEGIA— es un borrón: el texto curvo necesita unos 90 px de
diámetro para leerse. El nombre de la consultora sí queda accesible por el
atributo `alt` de la imagen y por el pie de página. Si prefieres que se lea
también a la vista, quita el atributo `hidden` de
`<span class="marca-texto">` en `index.html`: el emblema y el logotipo
tipográfico quedan uno junto al otro en un bloqueo horizontal. El CSS ya
contempla esa disposición.
...
página: dice **Compilación 2026-09-15.08**. Si dice otra cosa, o no dice nada,
...
Las rutas propias llevan `?v=2026-09-15.08`. Ese sufijo obliga al navegador y al
